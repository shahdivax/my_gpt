...
import torch
import torch.nn as nn
from transformers import PreTrainedTokenizerFast, PretrainedConfig, MODEL_FOR_CAUSAL_LM_MAPPING
from utils import load_config
import os
from tokenizers import ByteLevelBPETokenizer
...
def create_model(config):
    """Creates a custom language model from scratch based on the configuration."""
    hf_config = CustomConfig(**config["model"])
    model = CustomLanguageModel(hf_config)
    return model

def get_tokenizer(config):
    """Loads a trained ByteLevelBPE tokenizer and wraps it for HF compatibility."""
    model_path = config["tokenizer"]["model_path"]
    if not os.path.exists(os.path.join(model_path, "vocab.json")):
        raise ValueError(f"No tokenizer found at {model_path}. Please train the tokenizer first.")
    ...
    print(f"Tokenizer loaded successfully. Vocab size: {hf_tokenizer.vocab_size}")
    return hf_tokenizer

# Register the model with Hugging Face
MODEL_FOR_CAUSAL_LM_MAPPING.register(CustomConfig, CustomLanguageModel)
